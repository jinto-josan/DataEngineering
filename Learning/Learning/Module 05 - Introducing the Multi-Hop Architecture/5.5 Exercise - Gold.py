...
# MAGIC * Join `StopTransaction` Requests and `StartTransaction` Responses, matching on `transaction_id` (left join)
...
# MAGIC %md
# MAGIC ### EXERCISE: Join StopTransaction Requests and StartTransaction Responses
# MAGIC In this exercise, [left join](https://spark.apache.org/docs/3.1.2/api/python/reference/api/pyspark.sql.DataFrame.join.html) `StopTransaction` Requests and the newly joined `StartTransaction` Request/Response DataFrame (from the previous exercise), matching on `transaction_id` (left join).
# MAGIC
# MAGIC Target Schema:
# MAGIC
# MAGIC <br/>
...
# MAGIC %md
# MAGIC So we have calculated both the total time and total charge dispensed (total energy). This marks the end of this exercise. Lets do the clean up below
...
# MAGIC ## Wrap-up
# MAGIC
# MAGIC Congratulations, you have applied the medallion or multi-hop artechitecture using the bronze, silver and gold layer. It's handy to batch processing. Notice that you've read/written Parquet files through the process.
# MAGIC
# MAGIC Let's move on in this great learning journey to the next module. You are doing a great job!
...
# MAGIC |-------------------------------------------------------------------------|-------------------------------------------------------------------|
# MAGIC | <a href="$./5.4 Exercise - Silver" target="_self">Exercise - Silver</a> | <a href="$./5.6 Data-Quality" target="_self">Data Quality</a>     |
