...
# MAGIC
# MAGIC In this lesson you will learn how to shape the data to perform advanced analysis.
# MAGIC
# MAGIC ## Learning Objectives
# MAGIC By the end of this lesson, you should be able to:
# MAGIC * Know how to transform data schemas
# MAGIC * Know how to unpack nested structures
# MAGIC * Know how to join two data frames
# MAGIC * Know how to transform data values
# MAGIC
# MAGIC 🕒 **Estimated Time For Completion:** 2 hours
# MAGIC    
...
# MAGIC %md
# MAGIC ## Recap of the EV domain
# MAGIC
# MAGIC Recall that we did the following in the previous module :
# MAGIC
# MAGIC - We introduced the EV Domain and understood the importance of charge point data.
# MAGIC - We did a quick exercise where we converted timestamp in string format to timestamp format for ease of usage and computations later on.
# MAGIC - We also found out the most recent messages per charge point as an exercise.
# MAGIC
# MAGIC Proceeding ahead in the context of our EV domain, lets understand what is a **Charge Data Record** (to be called as **CDR** henceforth). A CDR is an important piece of information required by a **Charge Point Operator (CPOs)** to invoice a customer for the charge dispensed in a transaction. CDR records contain the following information
# MAGIC
# MAGIC | CDR field | Description |
# MAGIC | --- | --- |
# MAGIC | total_energy | How much energy was dispensed (StopTransactionRequest.meter_stop - StartTransactionRequest.meter_start) |
# MAGIC | total_time |  How long the transaction was (StopTransactionRequest.timestamp - StartTransactionRequest.timestamp) | 
# MAGIC
# MAGIC In this exercise we want to get the total Energy dispensed and the total time taken by vehicle to charge itself.
# MAGIC
# MAGIC So lets get started to find out the above two values from the data provided.

# COMMAND ----------

# MAGIC %md
# MAGIC We will do this calculation from our OCPP Event data. After the Charge Point has registered itself with the CSMS (Charging Station Management System), it sends information via the OCPP protocol about the Transactions, in the following order:
# MAGIC
...
# MAGIC
# MAGIC
# MAGIC **NOTE:** The example payloads are decorated with a few extra fields including the `message_id`, `message_type`, and `charge_point_id` which generally comes from the CSMS. The actual payloads from the Charge Point come in the `body` field encoded as a json string.
# MAGIC
# MAGIC
# MAGIC
...
# MAGIC Before we get started, we need to quickly set up this notebook by installing a set of helpers, cleaning up your unique working directory (as to not clash with others working in the same space), and setting some variables. Run the following cells using `Shift + Enter`. 
# MAGIC
...
# MAGIC %md
# MAGIC Ignore the warnings to restart Python, if you get any, in the cell above..
...
# MAGIC %md
# MAGIC In the last connection time exercise of the previous module, we already learned how to read in data (and we did it!). Let's read it in again, but this time, to make it easy, it's already filled out. Run the next two cells below to read in our OCPP data.
...
# MAGIC
# MAGIC **Note** Please ensure that the new column is named `new_body` because our tests are expecting that name in their asserts. Is that good naming practice? You tell us!
# MAGIC
# MAGIC <br/>
# MAGIC
# MAGIC ```
...
# MAGIC
# MAGIC Make sure to return the following columns using the [select](https://spark.apache.org/docs/latest/api/python/reference/pyspark.sql/api/pyspark.sql.DataFrame.select.html) function:
...
# MAGIC Now that we have our `StartTransaction` events joined together, we can now join our Data Frame with the `StopTransaction` Request data that contains `meter_stop` and `timestamp`.
# MAGIC
...
# MAGIC
# MAGIC **HINT**: You can convert a timestamp type to seconds using the [cast](https://spark.apache.org/docs/latest/api/python/reference/pyspark.sql/api/pyspark.sql.Column.cast.html?highlight=cast) function and the `long` type. Of course, that's just seconds.
...
# MAGIC %md
# MAGIC ### EXERCISE: Calculate total_energy and total_time
...
# MAGIC %md
# MAGIC ## Wrap-Up & Clean-Up
# MAGIC
# MAGIC Congratulations, you have now reached our goal of calculating the total time for which the vehicle was charging and the total charge dispensed by the charge point.
# MAGIC
# MAGIC Let's do the clean-up before moving to the next module.
...
# MAGIC | Next Module                                                                                                                                  |
# MAGIC |----------------------------------------------------------------------------------------------------------------------------------------------|
# MAGIC | <a href="$../Module 05 - Introducing the Multi-Hop Architecture/5.0 Table of Contents" target="_self">Introducing the Multi-Hop Architecture</a>  | 
# MAGIC
