...
-- MAGIC * Have a SQL warehouse access for UDFs (User Defined Functions)
-- MAGIC
-- MAGIC **Note** The above 2 accesses will already be provided to you while adding you to the workspace by your trainer. Please get in touch with your trainer if you still face any issues while running the below cells.
...
-- MAGIC %md
-- MAGIC ### Setup Validation
-- MAGIC To validate that the above set up scripts has run successfully, the script should have successfully created the `current_catalog` as `hive_metastore` and `Your Catalog` as a value including your user name (for e.g something like `YOUR_USER_NAME_SOME_OTHER_CHARACTERS`). One you have ensured and observed in the output above, you are good to proceed from here. 
-- MAGIC
-- MAGIC Also, to give a little more context, the above set up is needed to create a working directory, a database as well as a few datasets to be used in this notebook.
...
-- MAGIC
-- MAGIC Run the **`CREATE`** statement below, and click the **Catalog** (_Ctrl + Option +C_) icon in the left sidebar to confirm this new catalog was created.
...
-- MAGIC
-- MAGIC Run the code below, and again click the Catalog (Ctrl + Option +C) icon in the left sidebar to confirm this new schema (named as `example`) was created.
...
-- MAGIC
-- MAGIC Run the cells below, and click the **Catalog** icon in the left sidebar to explore the contents of the *example* schema. Note that we don't need to specify three levels when specifying the table or view names below, since we selected a default catalog and schema.
...
-- Once done, check if the table is created in the Catalog/<YOUR_CATALOG_NAME>/example location
...
-- MAGIC
-- MAGIC 1. Right-click on **SQL** section on the sidebar and open the **SQL Editor** link in a new tab.
-- MAGIC 1. Select the `Test SQL Warehouse Pro` warehouse by choosing it from the drop down at the top right handside of the SQL Editor tab. If the SQL warehouse is not started, go to the `SQL` section again in the sidebar, open `SQL Warehouses` link in another tab and start `Test SQL Warehouse Pro`. If the warehouse is not present at all, please feel free to create a small warehouse of your own or get in touch with your trainer to help you out.
...
-- MAGIC * A catalog beginning with the prefix *dbacademy_* (or your username), which is the one we created earlier in this notebook.
...
-- MAGIC | Next Module                                                                                                |
-- MAGIC |------------------------------------------------------------------------------------------------------|
-- MAGIC | <a href="$../Module 12 - Final Project/12.0 Table of Contents" target="_self">Final Project</a> |
-- MAGIC
