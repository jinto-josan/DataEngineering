...
# MAGIC In lieu of real streaming infrastructure (like Kafka, which we won't set up for financial reasons here in databricks), we'll use the Spark Streaming Rate source to stream data into our notebook. This Rate Stream has two fields, `timestamp` and `value` (0..n). Reading our data into a Pandas DataFrame allows us to leverage the DataFrame's index (0..n) to create a column to join the Rate Stream `value` field/column with the sample data's index column. 

# COMMAND ----------

# MAGIC %md
# MAGIC But first, let us create some mock data
...
def select_columns(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df=(input_df.select("charge_point_id", "new_body.status", "new_body.timestamp")
...
def aggregate_window_watermark(input_df: DataFrame) -> DataFrame:
    ...
    input_df= (
        ...
        #number of status updates in window period
        .agg(
            F.count("status")
        )
    )
...
# MAGIC |--------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------|
# MAGIC | <a href="$./7.09 Stateful Streaming in a Nutshell" target="_self">7.09 Stateful Streaming in a Nutshell</a> | <a href="$../Module 08 - Data security and privacy/8.0 Table of Contents" target="_self">8.0 Table of Contents |
# MAGIC
